...
import { auth, db } from "./firebase";        // <-- tambahkan db di sini
...
function ProtectedRoute({ user, children }) {
  // undefined = masih loading status auth
  if (user === undefined) {
    return <div style={{ textAlign: "center", marginTop: 80 }}>Memuat…</div>;
  }
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route
          ...
          element={
            user
              ? (profile ? <Navigate to="/dashboard" replace /> : <Navigate to="/register" replace />)
              : <Login />
          }
        />
        <Route
